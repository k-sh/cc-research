...
研究プロジェクトのディレクトリで、次のコマンドを実行してください:

```
npx github:k-sh/cc-research
```

`.claude/skills/` 配下に 15 個のスキルがインストールされます。Claude Code はプロジェクト内の
`.claude/skills/*/SKILL.md` を自動的に検出し、スラッシュコマンドとして利用可能にします。
...
既にスキルが存在する場合は既定でスキップされます(上書きしたい場合は `--force` を付ける)。
書き込み前に内容を確認したい場合は `--dry-run` を、カレントディレクトリ以外にインストールしたい
場合は `--dir <path>` を指定してください(`npx github:k-sh/cc-research -- --dry-run` のように
`--` を挟む必要がある場合があります)。オプション一覧は `--help` で確認できます。

このリポジトリを直接 Claude Code のプロジェクトディレクトリとして開くか、`.claude/skills/` を
手動でコピーしても構いません。

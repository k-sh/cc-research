...
- **研究記録は「常に覚えておくべきもの」と「必要になったら調べればよいもの」を仕分けて管理する。**
  研究が長く続くほど蓄積される情報は増えるが、そのすべてを毎回思い出そうとすると、肝心なものが
  埋もれる。次の 5 種類は毎回の検討・実装の前提として常に効いてくるため、`research/STATUS.md`
  の「常に確認する情報」に 1〜2 行の要点と参照先だけを載せ、**検討を進めるとき・実装するときは
  必ずこのセクションを確認してから着手する**:
  - **目的**(objective.md): 研究01で設定し、以降は基本的に不変。ぶれると全工程の判断基準が崩れる
  ...
  - **評価方法と結果の整理形式**(evaluation-metrics.md・evaluation-report.md): 何をもって
    良し悪しを判定するか、結果をどのファイル形式・目次構成で整理するか。指標の定義自体は
    research-01 が設定するが、実際の整理形式(ファイル構成・見出し立て)は research-09 が
    最初の評価実験で確定させ、以降の研究(research-10 のアブレーション結果、追加実験)も
    同じ形式に揃える。形式がその都度ばらつくと、過去の結果と見比べるたびに構造から
    読み解き直すコストがかかる
  - 一方、**過去の経緯(不採用にした案とその理由、検討の紆余曲折)や使わなくなったツールの
    使い方は、この「常に確認する情報」には載せない。** method-proposals.md の不採用理由や
    literature-review.md 等、元の詳細ファイルに残しておき、必要になったとき(査読対応で
    「なぜこの案を採らなかったか」を聞かれた等)にだけ参照しに行けば十分
  - **書きっぱなしにせず、定期的に棚卸しする。** 目安は数ターン(会話のやり取りが何度か
    続いた頃、または新しいフェーズ・新しい skill に入る前)に一度。「常に確認する情報」の
    各欄がまだ実態と合っているか(設計や方針が変わって古くなっていないか)、逆に本来ここに
    載るべき決定(重大なバグ、ツールの使い方の変更等)が記録されないまま漏れていないかを
    見直し、必要なら更新して見出しの「最終棚卸し」日付を更新する。棚卸しを怠ると、この
    セクション自体が古い情報のまま放置され、「常に確認しているつもりで実は古い前提のまま
    進めていた」という本末転倒が起きる。逆に、棚卸しのたびに何でも追記して肥大化させない
    ことも重要(短い要約であることがこのセクションの価値なので、増える一方なら「本当に
    常に効いてくる情報か」を都度削る側にも倒す)
